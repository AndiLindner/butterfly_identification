...
def worker(args):
    ...
    # Targets -> used to stratify train-test-split and for class weights
    ...
    if ("resne" in args.model or "regne" in args.model):
        ...
        elif ("_s" in args.model or "_b" in args.model):
            block_channels=[96, 192, 384, 768]
        elif ("_l" in args.model):
    ...
    for epoch in range(epochs):
        ...
        # Calculate average losses per sample (considering last batch cut)
        avgTrainLoss = totalTrainLoss / trainSamples
        avgValLoss = totalValLoss / valSamples

        # Calculate training and validation accuracy (considering last batch cut)
        trainCorrect = trainCorrect / trainSamples
        valCorrect = valCorrect / valSamples

        endTime = time.time()
        epochTime = endTime-startTime
        epochTime_clean = time.strftime('%d, %H:%M:%S', time.gmtime(epochTime))
        ...
        history['train_time'].append(epochTime)

        # Print some statistics
        if accelerator.is_main_process:
            print("Epoch: {}/{}".format(epoch+1, epochs))
            print(f"Training time: Day {epochTime_clean}")
        ...
        scheduler.step(avgValLoss)  # Metric ignored if not monitored by scheduler

    if accelerator.is_main_process:
        print(f"\nTraining done. Saving history and final model.\n")
